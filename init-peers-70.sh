...
# It is currently hardcoded to use 10.0.0.70 - 10.0.0.73 addresses for
...
echo "Description=udpproxy in from 10.0.0.7$NODE" >> $CREATE_PATH/proxy-in-$NODE.service
...
echo "Description=udpproxy to 10.0.0.7$NODE" >> $CREATE_PATH/proxy-out-$NODE.service
...
echo "Description=udpproxy in from 10.0.0.7$NODE" >> $CREATE_PATH/proxy-in-$NODE.service.pc
...
echo "Description=udpproxy to 10.0.0.7$NODE" >> $CREATE_PATH/proxy-out-$NODE.service.pc
...
ADDRESS_DIGIT=$NODE
OUTGOING_ADDRESS=10.0.0.7$ADDRESS_DIGIT
...
cp $CREATE_PATH/out-$NODE.key 10.0.0.7$NODE/in-$LOOP.key
echo 1 > 10.0.0.7$NODE/in-$LOOP.count
}
...
do
 CREATE_PATH="10.0.0.7$ERASE_LOOP"
